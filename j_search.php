 ...
 if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['search']) && $_POST['search'] == "search") )
		{
			...
			if(trim($type)<>"")
			 ...
			 if($s==1)
			  {
			  $where=$where ." AND  `job_nature`='$type'";
			  }
			 else
			  {
			  $where=$where ." and  `job_nature`='$type'";
			...
			if(trim($experience)<>"")
			 ...
			 if($s==1)
			  {
				if($experience==0)
				 $where=$where ." AND  (`exp_req`='none' or `exp_req`<='$experience') ";
				else
			  $where=$where ." AND  `exp_req`<='$experience' ";
			  }
			 else
			  {
			  if($experience==0)
			...
			// search
			 ...
			 $array_data=array();
			 $d=date('Y-m-d');
				$qry = "select * from $com_job_post where `accepted`='yes' and `post`='1' and `continue`='1' and `deadline`>='$d' $where order by id desc ";
	...
	<head>
		...
		<script>
			function show(){
					...
					xmlhttp.open("POST","j_custome_search.php",true);
	...
	<body>
		...
		<?php
			if($flag==0){
			 ...
			 $array_data=array();
			 $d=date('Y-m-d');
				$qry = "select * from $com_job_post where `accepted`='yes' and `post`='1' and `continue`='1' and `deadline`>='$d' order by id desc";
				$result_all = mysql_query($qry);
				if ($result_all) {
					while ($data = mysql_fetch_array($result_all)) {
						$array_data[] = $data;
		...
		<div class="row">
			...
			<a href="j_se_home.php"><button class="btn btn-primary" type="button"style="margin-bottom:20px;">Home</button></a>
			<!--form start -->
